...
export class PaintCalculatorService {
  ...
  doorValidation(wallHei: ElementRef, doorCount: ElementRef){
    ...
    wallToElements(wallWid: ElementRef, wallHei:ElementRef, winCount: ElementRef, doorCount: ElementRef, windInput: ElementRef, doorInput: ElementRef){
      const winDoorArea = this.winDoorArea(this.elementToNumber(winCount), this.elementToNumber(doorCount));
      const wallTotalArea = this.wallArea(this.elementToNumber(wallWid), this.elementToNumber(wallHei));
      if(winDoorArea > (wallTotalArea / 2) || (wallTotalArea / 2) == 0){
        alert('A quantidade de janelas e portas excedem o limite aceitável!')
    ...
    painCount(walls: number[]){
      ...
      return `Para Pintar essa Sala é preciso ${biggesTin} latas de 18L, ${bigTin} latas de 3.6L, ${mediumTin} latas de 2.5L e ${smallTin} latas de 0.5L!`
    }
}
